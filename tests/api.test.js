...
describe('text-similarity-node', () => {
  ...
  describe('Basic Similarity Functions', () => {
    ...
    test('calculateSimilarity - Jaccard with n-grams', () => {
      const result = calculateSimilarity('hello', 'hallo', AlgorithmType.JACCARD, {
    ...
    test('calculateSimilarity - Cosine with words', () => {
      const result = calculateSimilarity('hello world', 'world hello', AlgorithmType.COSINE, {
    ...
    test('calculateSimilarity - Jaro-Winkler', () => {
      const result = calculateSimilarity('martha', 'marhta', AlgorithmType.JARO_WINKLER, {
        prefixWeight: 0.1,
        prefixLength: 4,
      });
    ...
    test('calculateSimilarity - Tversky with custom weights', () => {
      const result = calculateSimilarity('hello', 'hallo', AlgorithmType.TVERSKY, {
        alpha: 0.5,
        beta: 0.5,
        preprocessing: PreprocessingMode.NGRAM,
        ngramSize: 2,
    ...
    test('calculateDistance - Levenshtein', () => {
      const result = calculateDistance('kitten', 'sitting', AlgorithmType.LEVENSHTEIN);
  ...
  describe('Batch Operations', () => {
    ...
    test('calculateSimilarityBatch - synchronous', () => {
      const results = calculateSimilarityBatch(testPairs, AlgorithmType.LEVENSHTEIN);
    ...
    test('calculateSimilarityBatch - with different algorithms', () => {
      const results = calculateSimilarityBatch(testPairs, AlgorithmType.JACCARD, {
        preprocessing: PreprocessingMode.NGRAM,
        ngramSize: 3,
      });
  ...
  describe('Asynchronous API', () => {
    ...
    test('calculateDistanceAsync - Promise-based', async () => {
      const distance = await calculateDistanceAsync('kitten', 'sitting', AlgorithmType.LEVENSHTEIN);
    ...
    test('calculateSimilarityBatchAsync - Promise-based batch', async () => {
      ...
      const results = await calculateSimilarityBatchAsync(testPairs, AlgorithmType.LEVENSHTEIN);
    ...
    test('calculateSimilarityAsync - error handling', async () => {
      ...
      await expect(calculateSimilarityAsync(largeString1, largeString2)).rejects.toThrow();
    });
  });
  ...
  describe('Configuration Management', () => {
    ...
    test('Case insensitive comparison', () => {
      ...
      const result = calculateSimilarity('Hello', 'HELLO', AlgorithmType.LEVENSHTEIN);
  ...
  describe('Unicode Support', () => {
    test('Unicode characters - Basic Latin Extended', () => {
      const result = calculateSimilarity('café', 'cafe', AlgorithmType.LEVENSHTEIN);
    ...
    test('Unicode characters - Greek', () => {
      const result = calculateSimilarity('αβγ', 'αβδ', AlgorithmType.LEVENSHTEIN);
    ...
    test('Unicode case insensitive - Mixed scripts', () => {
      ...
      const result = calculateSimilarity('Café', 'CAFÉ', AlgorithmType.LEVENSHTEIN);
  ...
  describe('Performance and Memory', () => {
    ...
    test('clearCaches', () => {
      ...
      for (let i = 0; i < 10; i++) {
        calculateSimilarity(`test${i}`, `test${i + 1}`, AlgorithmType.LEVENSHTEIN);
      }
    ...
    test('Performance - Large batch processing', () => {
      ...
      const results = calculateSimilarityBatch(largeBatch, AlgorithmType.LEVENSHTEIN);
  ...
  describe('Error Handling', () => {
    ...
    test('Invalid configuration', () => {
      const result = calculateSimilarity('hello', 'world', AlgorithmType.TVERSKY, {
    ...
    test('One empty string', () => {
      const result = calculateSimilarity('hello', '', AlgorithmType.LEVENSHTEIN);
  ...
  describe('Algorithm-Specific Tests', () => {
    test('Damerau-Levenshtein vs Levenshtein with transpositions', () => {
      ...
      const levenshtein = calculateDistance(str1, str2, AlgorithmType.LEVENSHTEIN);
      const damerau = calculateDistance(str1, str2, AlgorithmType.DAMERAU_LEVENSHTEIN);
    ...
    test('Jaccard vs Sorensen-Dice comparison', () => {
      ...
      const dice = calculateSimilarity(str1, str2, AlgorithmType.SORENSEN_DICE, {
        preprocessing: PreprocessingMode.WORD,
      });
  ...
  describe('Edge Cases and Stress Tests', () => {
    test('Very long strings', () => {
      ...
      const result = calculateSimilarity(longStr1, longStr2, AlgorithmType.LEVENSHTEIN);
    ...
    test('Identical strings', () => {
      const result = calculateSimilarity('identical', 'identical', AlgorithmType.LEVENSHTEIN);
    ...
    test('Completely different strings', () => {
      const result = calculateSimilarity('abcdef', 'xyz123', AlgorithmType.LEVENSHTEIN);
    ...
    test('Special characters and punctuation', () => {
      const result = calculateSimilarity('Hello, world!', 'Hello world', AlgorithmType.LEVENSHTEIN);
...
describe('Performance Benchmarks', () => {
  ...
  test('Benchmark - Small strings (10-50 chars)', () => {
    ...
    console.log(`Small strings benchmark: ${endTime - startTime}ms for ${pairs.length} pairs`);
  });
  ...
  test('Benchmark - Medium strings (100-500 chars)', () => {
    ...
    console.log(`Medium strings benchmark: ${endTime - startTime}ms for ${pairs.length} pairs`);
  });
  ...
  test('Benchmark - Async vs Sync performance', async () => {
    ...
    const syncResults = calculateSimilarityBatch(testPairs, AlgorithmType.LEVENSHTEIN);
    const syncEnd = process.hrtime.bigint();

    // Async benchmark
    const asyncStart = process.hrtime.bigint();
    const asyncResults = await calculateSimilarityBatchAsync(testPairs, AlgorithmType.LEVENSHTEIN);
    ...
    console.log(`Sync time: ${syncTime.toFixed(2)}ms, Async time: ${asyncTime.toFixed(2)}ms`);
