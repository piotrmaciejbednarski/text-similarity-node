...
async function runBenchmark(groupName, libraries, timeMs = 1000) {
  ...
  if (bench.tasks.length > 0) {
    ...
    const sortedByOps = [...results].sort((a, b) => (b.opsPerSec || 0) - (a.opsPerSec || 0));
    ...
    if (sortedByOps[0]?.opsPerSec) {
      ...
      console.log('Winner: Could not determine due to missing benchmark results');
    }
  }
}
...
async function benchmarkLevenshtein() {
  ...
  const libraries = [
    ['text-similarity-node', () => textSimilarity.similarity.levenshtein(s1, s2)],
    ['string-comparison', () => stringComparison.levenshtein.similarity(s1, s2)],
...
async function benchmarkJaroWinkler() {
  ...
  const libraries = [
    ['text-similarity-node', () => textSimilarity.similarity.jaroWinkler(s1, s2)],
    ['string-comparison', () => stringComparison.jaroWinkler.similarity(s1, s2)],
...
async function benchmarkJaccard() {
  ...
  const libraries = [
    ['text-similarity-node (char)', () => textSimilarity.similarity.jaccard(s1, s2, false)],
    ['text-similarity-node (word)', () => textSimilarity.similarity.jaccard(s1, s2, true)],
    ['string-comparison', () => stringComparison.jaccardIndex.similarity(s1, s2)],
  ];
...
async function benchmarkCosine() {
  ...
  const libraries = [
    ['text-similarity-node (char)', () => textSimilarity.similarity.cosine(s1, s2, false)],
    ['text-similarity-node (word)', () => textSimilarity.similarity.cosine(s1, s2, true)],
...
async function benchmarkDice() {
  ...
  const libraries = [
    ['text-similarity-node (char)', () => textSimilarity.similarity.dice(s1, s2, false)],
    ['text-similarity-node (word)', () => textSimilarity.similarity.dice(s1, s2, true)],
    ['string-similarity', () => stringSimilarity.compareTwoStrings(s1, s2)],
    ['string-comparison', () => stringComparison.diceCoefficient.similarity(s1, s2)],
  ];
...
function printSystemInfo() {
  ...
  if (global.gc) {
    ...
    console.log('Garbage collection: DISABLED (run with --expose-gc for better accuracy)');
  }
